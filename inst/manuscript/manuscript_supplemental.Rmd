...
There are multiple 16S rRNA databases whose maintainers use different curation approaches resulting in different taxonomic and sequence compositions as well as being differently formatted. 
One can perform taxonomic or other 16S rRNA sequence analysis on an individual databases but with metagenomeFeatures and MgDb annotation packages the same analysis can easily be performed on multiple databases, increasing the power of the analysis.
...
The Greengenes database taxonomic name is more rigidly formatted, only including the major taxonmic levels, e.g. does not include intermediate level such as suborder, and only goes to the species level. 

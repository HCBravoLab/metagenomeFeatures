...
subtitle: "_Paenibacillus_ species resolution for 16S rRNA V12 and V4 regions."
...
# library(silva128.1MgDb, lib.loc = "~/Documents/Fall2018/installed_packages/"); packageVersion("silva128.1MgDb")
...
First subset the databases using the `mgDb_select()` function.
Then we will summarize the taxonomy data using functions from `tidyverse` package, specifically `dplyr`, `stringr` and `forcats` functions for manipulating `data.frames`, `strings`, and `factor` vectors respectively. 

__TODO__ Clean-up do not want to load both taxa and seq into memory
...
## Per genus count data - greengeness
...
__TODO__ Revise text - all three databases
For the Greengenes 13.5 database, there are a total of `r total_otus_greengenes` sequences classified as `r nlevels(taxa_df_greengenes$Species)` species in the Genus _Paenibacillus_.
...
Sequences only classified to the genus level, "Unassigned", is the most abundant group, `r no_species_assignment_greengenes`.

Similarly, we extracted _Paenibacillus_ sequences from Silva 128 database. Fig. \@ref(fig:speciesCountSilva) shows the number of species in the Genus _Paenibacillus_ in the Silva database. 

For the RDP 11.5 database, all the sequences belonging to _Paenibacillus_ genus were either uncultured, unidentified, or unassigned. 
...
Here, we work with sequences only in Greengenes 13.5 database for simplicity, the same analysis can be done with any other database.
...
Demonstrate process using gg and V12. 
Code hidden for other databases and V4, see Rmarkdown for full code, __PATH to Rmarkdown in repo__.

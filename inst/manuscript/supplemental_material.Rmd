...
- Multiple 16S rRNA databases
- Can perform analysis on an individual database
...
library(silva128.1MgDb); packageVersion("silva128.1MgDb")
library(ribosomaldatabaseproject11.5MgDb); packageVersion("ribosomaldatabaseproject11.5MgDb")
```
...
- number of database entries and fraction of entries with RNAcentralIDs
- note that the mgDb_taxa slot is a tbl_dbi
```{r echo = FALSE}
...
get_size_and_rnacentral <- function(mgdb){
  ...
  data_frame(n_entries = db_size,
             n_entries_rnaid = db_wrna_id)
}

db_list <- list(gg = gg13.5MgDb,
                slv = slv128.1MgDb,
                rdp = rdp11.5MgDb)

db_info_df <- db_list %>%
  map_dfr(get_size_and_rnacentral, .id = "mgdb")
db_info_df %>% mutate(frac_w_rnaids = (n_entries_rnaid)/n_entries)
```


- RNAcentral id overlap
```{r}
...
```{r speciesCount, fig.cap = "Number of sequences in the Greengenes database assigned to species in the genus _Paenibacillus_ for the SILVA and Greengenes databases."}
...
```{r distDFfuns, echo = FALSE, message = FALSE}
...
```{r getDistDF, echo = FALSE, message = FALSE}
### GG
gg_for_seq_comp <- tidy_gg_taxa %>% filter(!is.na(rnacentral_ids), Species != "Unassigned")

### SILVA
slv_for_seq_comp <- tidy_slv_taxa %>% filter(!is.na(RNAcentralID), Species != "Unassigned")

### RDP
rdp_for_seq_comp <- tidy_rdp_taxa %>% filter(!is.na(RNAcentralID), Species != "Unassigned")
...
taxa_list <- list(gg = gg_for_seq_comp,
                  slv = slv_for_seq_comp,
...
```{r pairDist, fig.cap = "Distribution of within and between Species pairwise distances for the V12 and V4 16S rRNA region. Sequences not classified to the Species level were excluded from the analysis."}
multidb_dist_df  %>%
  # filter(Database == "gg") %>%
    ggplot(aes(x = group_comp, y = distance, fill = Database)) +
    geom_boxplot() +
    facet_grid(.~amplicon) +
    labs(x = "Species Comparison", fill = "Database") +
        theme_bw() +
...
multidb_dist_df %>%
  filter(Keys_Species %in% c("polymyxa", "amylolyticus", "lautus","chondroitinus"),
         Keys2_Species %in% c("polymyxa", "amylolyticus", "lautus","chondroitinus")) %>%
    ggplot() +
    geom_boxplot(aes(x = amplicon,
                    y = distance,
                    fill = Database)) +
    facet_grid(Keys_Species~Keys2_Species) +
   theme_bw()
```

...
vignette_assay_data <- function(){
    system.file("extdata", "msd16s_counts.csv",
...
vignette_pheno_data <- function(){
    system.file("extdata", "msd16s_sample_data.csv",
                package="metagenomeFeatures")  %>%
        read.csv(row.names = 1, stringsAsFactors = F) %>%
        Biobase::AnnotatedDataFrame()
}

vignette_msd16s_MgDb <- function(){
    ...
    db_seq <- system.file("extdata", "msd16s_MgDb_seq.fasta.gz",
                          package="metagenomeFeatures") %>% readDNAStringSet()

    new("MgDb",
            seq = db_seq,
            taxa_file = system.file("extdata", "msd16s_MgDb_taxa.sqlite",
                                    package="metagenomeFeatures"),
